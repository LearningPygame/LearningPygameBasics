...
class Ball:
        def __init__(self, radius, y, x, color, size, maxforce, force, life):
                ...
                self.force=force                        # criação da definiçao de Ball
                self.radius=radius
                self.color=color
                self.life=life
        ...
        def fall(self):
                if self.y < h-self.radius:
                        ...
                        if self.force < self.maxforce:
                                self.force+=1
                elif self.y > h-self.radius or self.y == h-self.radius:             #fisica da Ball
...
ball = []                        #array para Ball e atribuição de valores a Ball
...
while 1:
        ...
        if z == 1:
                ...
                z = 3              #cada cor de uma bola dif da outra
        elif z > 1:
                z-=1
